...
{
    public function AddToCompere(Request $require, $compere_id)
    {
        if (Auth::check()) {
      $exists = Compare::where('user_id',Auth::id())->where('product_id',$compere_id)->first();

            if (!$exists) {
               Compare::insert([
                'user_id' => Auth::id(),
                'product_id' => $compere_id,
    ...
    public function RemoveCompere($id){
        Compare::where('user_id',Auth::id())->where('id',$id)->delete();
        
        return response()->json(['success'=>'Compare Product Successfully Remove']);
    }
}
